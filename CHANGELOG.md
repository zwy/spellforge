...
- **桌面版在系统代理环境下启动失败**：就绪探测改用强制直连
  （`ProxyHandler({})`），不再经过 macOS 系统代理 / 环境代理。
  部分代理工具会 503 掉 urllib 风格的回环请求，导致 v1.1.0 双击启动时
  误报「就绪探测失败」；服务本身实际已正常启动。新增回归测试：
  在死代理环境变量 + 无 `no_proxy` 豁免的场景下完成启动 / 探测 / 停止。
